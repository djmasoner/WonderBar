	...
	function addProjectClasses(proj_id) {
		try {
			var x = document.getElementById("proj_classes");
			var cbs = x.getElementsByTagName("input");
			var rowCount = cbs.length;

			for(var i=1; i<rowCount; i++) {
				if(null != cbs[i] && true == cbs[i].checked) {
					var class_id = cbs[i].value;
					...
					var userRequest = new Request("https://wonderbar-cs467.ue.r.appspot.com/dbprojects/" + proj_id + "/dbclasses/" + class_id, initObject);
	...
	function addMaterialToProject(proj_id) {
		try {
			...
			for(var i=1; i<rowCount; i++) {
				...
				let initObject = {
					method: 'POST', body: JSON.stringify({"name":material_name, "quantity":material_quantity} /*, headers: reqHeader*/)
				};
	...
	function addPromptToProject (proj_id){
		...
		for(var i=1; i<=promptCount; i++) {
			...
			let initObject = {
				method: 'POST', body: JSON.stringify({"subheading":prompt_subheading, "description":prompt_description, "response_type_id": prompt_type} /*, headers: reqHeader*/)
			};
	...
	function getProjectTopics() {
		...
		.then(topics => {
			...
			label.innerHTML = "Select topic(s) *";
			p.appendChild(label);
			row.appendChild(p);
	...
	function getProjectClasses() {
		...
		.then(classes => {
			...
			p = document.createElement("p");
			var label = document.createElement("label");
			label.innerHTML = "Select class(es) *";
			p.appendChild(label);
			row.appendChild(p);
			...
			for (var i=0; i<rowCount;i++){
				...
				element1.type = "checkbox";
				element1.style.margin = "0 1%"
				element1.id = "class" + (i+1);
				element1.name="class" + (i+1);
				element1.value= classes["results"][i]["id"];
				element.innerHTML= classes["results"][i]["name"];
				element.htmlFor= "class" + (i+1);
				element2.appendChild(element1);
				element2.appendChild(element);
				row.appendChild(element2);
			}
	...
	function getProjectLocations() {
		...
		.then(locations => {
			...
			p = document.createElement("p");
			var label = document.createElement("label");
			label.innerHTML = "Select location(s) *";
			p.appendChild(label);
			row.appendChild(p);
